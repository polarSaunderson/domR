cat2 <- function(x, show = TRUE, name = NULL) {
  ...
  #'
  #' @description Using [cat2()] prints out a variable and it's name.
  #'
  ...
  #'
  #'   Sprinkling the [cat2()] function at key locations in functions allows us
  ...
  #'
  #'   If you just want the variable (and not the name), stick with `cat()`, or
  #'   use `cat3()`, which basically adds a newline `\n` before and after x.
  #'
  ...
  #'   [cat2()] calls to be added throughout a function, and a single variable
  ...
  #'   name of 'x' is used. However, if [cat2()] is nested, this will likely
